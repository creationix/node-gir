...
gir.load = function() {
    ...
    for (var subobj in obj) {
        ...
        var eventable = obj[subobj].__signals__ != undefined && makeArray(obj[subobj].__signals__).length > 0;
        if (eventable) {
            ...
            switch (prop) {
                case '__methods__':
                    for (var method_offset in obj[subobj][prop]) {
                        ...
                        if (obj[subobj].prototype[method_name] != undefined) {} //debug:console.warn("[node-gir] " + subobj + " object provides it's own " + method_name + " method. Not replacing existing method. :-(");
                        else
